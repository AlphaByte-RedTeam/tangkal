    ...
    try {
      ...
      if (otherResults.length > 0) {
        ...
        otherResults.forEach((f) => {
          ...
          console.log(
            `${chalk.red.bold("TYPE:")} ${chalk.white.bold(f.name || f.type)}  ${chalk.gray(`(Severity: ${f.severity.toUpperCase()})`)}`,
          );
          console.log(
            `${chalk.cyan("FILE:")} ${chalk.white(f.file)}:${chalk.yellow(f.line || 0)}`,
          );
      ...
      if (vulnerabilities.length > 0) {
        ...
        vulnerabilities.forEach((v) => {
          ...
          console.log(
            chalk.green(
              `[SOLUTION]: Upgrade ${v.name}@${v.version} to ${v.name}@${fixedIn} to fix.`,
            ),
          );
          ...
          console.log(
            `${chalk.white("[")}${severityColor(v.severity.toUpperCase())}${chalk.white(" Severity]")} ${chalk.blue(links)}`,
          );
      ...
      if (options.nuke) {
        ...
        console.log(
          chalk.yellow(
            "Review manually or run with --nuke to delete files interactively.",
          ),
        );
      }
    } catch (error: any) {
