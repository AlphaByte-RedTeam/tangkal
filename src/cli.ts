  ...
  .action(async (directory, options) => {
    ...
    if (otherResults.length > 0) {
        ...
        otherResults.forEach(f => {
            console.log(chalk.cyan(`File: ${f.file}`));
            if (f.line && f.line > 0) console.log(chalk.white(`Line: ${f.line}`));
            
            // Description/Explanation
            const desc = f.description || 'Suspicious pattern detected.';
            console.log(chalk.yellow(`${desc}`));
            
            // Optional Code Snippet
            if (f.content && f.line && f.line > 0) {
                 console.log(chalk.gray(`Code: ${f.content.trim().substring(0, 80)}${f.content.length > 80 ? '...' : ''}`));
            }
            console.log(''); 
        });
    }
